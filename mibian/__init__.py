...
def impliedVolatility(className, args, callPrice=None, putPrice=None, high=500.0, low=0.0):
	...
	for i in range(10000):	# To avoid infinite loops
		# Near expiry (days-to-expiry ~ 0) the option price becomes
		# insensitive to volatility, so `estimate` never satisfies the
		# rounded-equality check below. Once high/low collapse to the same
		# float (bisection has hit its resolution limit), stop instead of
		# burning the remaining iterations on a price that can't change.
		if high - low < 1e-9:
			break
		mid = (high + low) / 2
		if mid < 0.00001:
