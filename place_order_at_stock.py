...
#
# Generic NFO Stock Survivor - Trend following options trading for any
# NFO-listed stock (e.g., TCS, RELIANCE, INFY, HDFCBANK).
#
# Sell-only mode: Sells PE when stock moves up, CE when stock moves down.
#
# Usage:
#   python3 place_order_at_stock.py <STOCK_NAME> <SYMBOL_INITIALS>
#       <PE_DIST:CE_DIST> <EXCHANGE> <PE_GAP:CE_GAP> <PE_RESET:CE_RESET>
#       <PE_QTY:CE_QTY> <PE_START:CE_START> <REQUEST_TOKEN>
#
# Example:
#   python3 place_order_at_stock.py TCS TCS26FEB 50:50 NFO 5:5 10:10
#       175:175 0:0 <token>
###############################################################################
...
try:
    ...
    logging.error("%s", e)
    logging.error(
        "Format -- python3 place_order_at_stock.py <STOCK_NAME> "
        "<SYMBOL_INITIALS> <PE_DIST:CE_DIST> <EXCHANGE> "
        "<PE_GAP:CE_GAP> <PE_RESET:CE_RESET> "
        "<PE_QTY:CE_QTY> <PE_START:CE_START> <REQUEST_TOKEN>"
    )
    logging.error(
        "Sample-- python3 place_order_at_stock.py TCS TCS26FEB "
        "50:50 NFO 5:5 10:10 175:175 0:0 token123"
    )
    quit()
...
def on_ticks_update(ws, ticks):  # noqa
    """Callback to receive ticks and execute orders based on stock price movement.
